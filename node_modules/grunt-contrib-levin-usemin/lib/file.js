...
var getBlocks = function (content) {
  ...
  //   * 3 : the output
  //
  var regbuild = /\/{0,2}<!--\s*build:(\w+)(?:\(([^\)]+)\))?\s*([^\s]+)\s*-->/;
  // end build pattern -- <!-- endbuild -->
  var regend = /\/{0,2}<!--\s*endbuild\s*-->/;
  ...
  lines.forEach(function (l) {
    ...
    if (block && endbuild) {
      last.comma=comma;//用于判断替换数组中js引用的时候是否需要添加逗号
      last.raw.push(l);
      sections.push(last);
      block = false;
    }
    ...
    if (block && last) {
      ...
      }else{//for inline js block replacement
        last.src.push(l);
        if(/,/gm.test(l)){
            comma=true;
        }else{
