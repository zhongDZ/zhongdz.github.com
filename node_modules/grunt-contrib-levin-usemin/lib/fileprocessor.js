...
var _defaultPatterns = {
  html: [
    ...
    [
      /['"]([^"']+)["'],{0,1}\/{2}<js_inline\/\>/gm,
      'Update the HTML to reference our concat/min/revved js inline script files'
    ],
  ],
  css: [
    [
...
var defaultBlockReplacements = {
  ...
  //替换js文件中数组中引用的js文件
  js_inline:function(block){
      var comma=block.comma?',':'';
      return '"'+block.dest+'"'+comma+js_inline;//js_inline用于标记需要替换的重名名的js文件
  }
};
...
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath) {
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      var res;
      if (self.filePrefixer) {
            res = match.replace(src, self.filePrefixer(filterOut(file)));
      } else {
        res = match.replace(src, filterOut(file));
      }
      if (srcFile !== file) {
        self.log(match + ' changed to ' + res);
      }
      return res.replace(js_inline,'');//去除添加的 注释标记
    });
  });
...
/***
 * @auther levin cao
 * @descrition Add the prefixer for static file link
 * @param filePrefixer
 */
FileProcessor.prototype.addFilePrefixer = function(filePrefixer) {
