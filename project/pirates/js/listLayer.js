...
var listLayer = cc.Layer.extend({
    ...
    initTop : function(){
        var top_bg = new getPosSprite(this, res.top_bg, size.width/2, size.height);
        top_bg.anchorY = 1;

        var rank_icon = new getPosSprite(this, res.rank_icon, 74, 1078);

        var rank_title = new getPosSprite(this, res.rank_title, 210, 1078);

        var rank_btn_1 = new getPosSprite(this, res.rank_btn_1, 375, 1078);

        var rank_btn_2 = new getPosSprite(this, res.rank_btn_2, 535, 1078);
    },
    initMiddle : function(){
        var listBg = new getPosSprite(this, res.listBg, size.width/2, size.height/2 - 10);
        var listBgSize = listBg.getBoundingBox();

        var rank_closeBtn = new getPosSprite(listBg, res.rank_closeBtn, listBgSize.width/2, 12);
    },
    init:function(){
        ...
        scrollView.setContentSize(cc.size(480, 750));//设置scrollView对象里显示内容的可见区域大小
     
        scrollView.x = (layer.width - 480)/2;//设置scrollView坐标位置
        scrollView.y = (layer.height - scrollView.height)/2;//scrollView.height = 350
