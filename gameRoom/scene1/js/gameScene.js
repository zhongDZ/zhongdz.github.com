...
var Layer1 = cc.Layer.extend({
    ...
    addAni:function(){
        ...
        var aniSpr = new cc.SpriteFrame(texture0,cc.rect(0,0,640,435));//注意 new  别缺少了!!!!!!!!!
        ...
        aniSprObj.y = 0;
        var sprFrames = [];
        sprFrames[0] = new cc.SpriteFrame(texture0,cc.rect(0,0,640,435));
        sprFrames[1] = new cc.SpriteFrame(texture1,cc.rect(0,0,640,435));
...
var Layer4 = cc.Layer.extend({
    ...
    initUI: function () {
        ...
        var _this = this;
        var actionBy = new cc.SkewBy(2, 180, 0);
        var actionByCall = new cc.callFunc(function(){
            _this.addAni();
        },this);
        var seq = new cc.sequence(actionBy,actionByCall);
...
var Layer7 = cc.Layer.extend({
    ...
    addAni:function(){
        var aniSprObjTag = 3;
        var texture0 = cc.textureCache.addImage(res.e1);//添加纹理到缓存
        var texture1 = cc.textureCache.addImage(res.e2);//添加纹理到缓存
        var texture2 = cc.textureCache.addImage(res.e3);//添加纹理到缓存
        var aniSpr = new cc.SpriteFrame(texture0,cc.rect(0,0,119,119));//注意 new  别缺少了!!!!!!!!!
        var aniSprObj = new cc.Sprite(aniSpr);
        this.addChild(aniSprObj,1,aniSprObjTag);
        aniSprObj.anchorY = .5;
        aniSprObj.x = size.width/2 - 45;
        aniSprObj.y = size.height - 230;
        var sprFrames = [];
        sprFrames[0] = new cc.SpriteFrame(texture0,cc.rect(0,0,119,119));
        sprFrames[1] = new cc.SpriteFrame(texture1,cc.rect(0,0,119,119));
        sprFrames[2] = new cc.SpriteFrame(texture2,cc.rect(0,0,119,119));
        var animation = new cc.Animation(sprFrames,0.6);
        var animate = new cc.animate(animation);
        var repeat = animate.repeatForever();
        aniSprObj.runAction(animate);
...
var Layer9 = cc.Layer.extend({
    ...
    initUI: function () {
        ...
        for(var i=0;i<info.length;i++){
            var spr = new TouchSpr(info[i].res, i);
            spr.x = info[i].x;
            spr.y = info[i].y;
            this.addChild(spr,1)
        }
