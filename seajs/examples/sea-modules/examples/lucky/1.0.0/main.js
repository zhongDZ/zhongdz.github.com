document.attachEvent && alert("这个例子不支持 Old IE 哦"),
define("examples/lucky/1.0.0/main", ["./data", "./lucky", "jquery", "jquery-easing", "./user"],
function(a) {
    var b = a("./data"),
    c = a("./lucky");
    c.init(b)
...
define("examples/lucky/1.0.0/lucky", ["jquery", "jquery-easing", "examples/lucky/1.0.0/user"],
function(a, b, c) {
    function d(a, b) {
    ...
    function f(a, b) {
        ...
        e = b.x - a.x,
        f = d(a, b),
        g = Math.ceil(((a.width + b.width) / 2 - f) / j),
        h = c > 0 ? Math.ceil(g * c / f) : Math.floor(g * c / f),
        i = e > 0 ? Math.ceil(g * e / f) : Math.floor(g * e / f);
        a.lucky ? (b._xMove += 2 * i, b._yMove += 2 * h) : b.lucky ? (a._xMove += -2 * i, a._yMove += -2 * h) : (a._yMove += -1 * h, b._yMove += h, a._xMove += -1 * i, b._xMove += i)
    }
    var g = a("jquery");
    a("jquery-easing");
    var h = a("examples/lucky/1.0.0/user"),
    i = 100,
    j = 4;
    c.exports = {
        users: [],
        init: function(a) {
            g("#container").css("background", "none"),
            this.data = a,
            this.users = a.map(function(b) {
                return new h(b, a[b])
            }),
            this._bindUI()
        },
        _bindUI: function() {
            ...
            var b = this,
            c = document.querySelector("#go");
            c.innerHTML = c.getAttribute("data-text-start"),
            c.addEventListener("click", a, !1),
            g("#lucky-balls").on("click", "li",
            function(a) {
                var c = g(a.target),
                d = c.text();
                b.addItem(d),
...
define("examples/lucky/1.0.0/user", ["jquery"],
function(a, b, c) {
    function d(a, b) {
        this.name = a,
        this.options = b || {},
    ...
    d.prototype.createEl = function() {
        this.el = f("<li>" + this.name + "</li>").appendTo("#balls"),
        this.width = this.el.width(),
        this.height = this.el.height()
    },
    d.prototype.move = function(a) {
        this.left = e(0, h - this.width),
        this.top = e(0, g - this.height),
        this.zIndex = e(0, m),
        this.reflow(a)
    },
    d.prototype.reflow = function(a, b) {
        ...
        b ? (this.el[0].style.left = this.left, this.el[0].style.top = this.top) : this.el.animate({
            left: this.left,
            top: this.top
        },
        e(n, o), "easeOutBack", a)
    ...
    d.prototype.reset = function() {
        ...
        this.lucky = !1,
        this.el[0].className = "",
        this.el[0].style.width = i + "px",
        this.el[0].style.height = j + "px",
        this.width = this.el.width(),
        this.height = this.el.height(),
        this._maxTop = g - this.height,
        this._maxLeft = h - this.width
    },
    ...
    d.prototype.bang = function() {
        ...
        this.width = k,
        this.height = l,
        this.left = (h - this.width) / 2,
        this.top = (g - this.height) / 2,
        this.el.animate({
            left: this.left,
            top: this.top,
            width: this.width,
