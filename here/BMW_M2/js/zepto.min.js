...
(function ($) {
    var init =function(obj, call) {
        var old = 0,
            ...
            tstart = function (e) {
                offh = getH();
                move.css({transition: 'transform 0ms', '-webkit-transition': '-webkit-transform 0ms'});
                old = e.touches[0].pageY;
                if (!currr && move.attr("data-move") == null) {
                    currr = 0;
                } else if (move.attr("data-move")) {
                    currr = parseInt(move.attr("data-move"))
                }
            },
            tmove = function (e) {
                cur = parseInt(e.touches[0].pageY - old);
                curr = currr + cur;
                move.css({
                    transform: 'translateY(' + curr + 'px)',
                    '-webkit-transform': 'translateY(' + curr + 'px)'
                });
                move.attr("data-move", curr);
            },
            tend = function (e) {
                move.css({transition: 'transform 300ms', '-webkit-transition': '-webkit-transform 300ms'});
                currr = curr;
                if (currr >= 0 && cur > 0) {
                    currr = 0;
                    move.css({transform: 'translateY(0px)', '-webkit-transform': 'translateY(0px)'});
                    move.attr("data-move", 0);
                } else if (currr <= offh) {

                    move.css({
                        transform: 'translateY(' + offh + 'px)',
                        '-webkit-transform': 'translateY(' + offh + 'px)'
                    });
                    move.attr("data-move", offh);
                    currr = offh;
        ...
        function getH(){
            return moveDiv.height() -  move.height();
        }
        moveDiv = obj;
        move = moveDiv.children();
        offh = getH();
        if (typeof(call.endFun) == "function") {
            endFun = call.endFun
        }
        if(call.reset){

        }else{
            moveDiv.off("touchstart,touchmove,touchend");
            setTimeout(function(){
    ...
    $.fn.contentMove = function (options) {
        var args = $.extend({
            reset: false,
            endFun: function () {
            }
